...
<head>
    ...
    <title>Customer Reviews - Kape Kita Coffee Shop</title>
...
<section class="reviews" id="reviews">
    <div class="container">
        <h1>Customer <span style="color: #b2744c;">Reviews</span></h1>
        
        <?php
        // Check if user is logged in
        if (isset($_SESSION['user_id'])) {
            echo '<button onclick="togglePopup()" class="btn btn-primary" style="margin-top: -10px; margin-bottom: 15px; background: #b2744c;">Add Review</button>';
        }
        ?>
        ...
        <div class="row">
            ...
            if (mysqli_num_rows($result) > 0) {
                ...
                while ($row = mysqli_fetch_assoc($result)) {
                    ?>
                    <div class="col-md-4 py-3 py-md-0 review-card">
                        <div class="card">
                            <div class="card-body">
                                <h3><?php echo $row['title']; ?></h3>
                                <h5><?php echo $row['author_name'] . ' / ' . date('d F Y', strtotime($row['created_at'])); ?></h5>
                                <p><?php echo $row['content']; ?></p>
                                <p><strong>Rating: <?php echo $row['rating']; ?>/5</strong></p>
                            </div>
                        </div>
                    </div>
                    <?php
